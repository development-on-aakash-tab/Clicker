...
{
	int resultcode,flag; 
    String macaddress,response,data,server_ip,ipaddress,enrollment_id;
	...
	public void onCreate(Bundle savedInstanceState)  
	{
		...
		else
		{
			...
			enrollment_id=SharedPreferenceConnector.readString(this,SharedPreferenceConnector.ENROLLMENT_ID, null);// Read Enrollment ID from shared prefernces.
			 new Network_OperationAsync().execute(); // call Asynchronous task to execute in back ground.
	 ...
	 class Network_OperationAsync extends AsyncTask<String, String, String> {
		   ...
		   //override method of AsyncTask,invoked on the UI thread immediately after the task is executed. 
			...
			protected String doInBackground(String... aurl)
			{
				...
				//publishProgress(ipaddress);// calling onProgressUpdate() method. 
				
				flag=3;
			    
				 con = new HttpConnection(); // defined object for HttpConnection class.
				 response = con.getConnection(server_ip, enrollment_id, macaddress,ipaddress, flag); // call the getConnection() method of HttpConnection class and response is collected.
					
				//response = con.getConnection(server_ip, enrollment_id, macaddress, flag); // call the getConnection() method of HttpConnection class and response is collected.
