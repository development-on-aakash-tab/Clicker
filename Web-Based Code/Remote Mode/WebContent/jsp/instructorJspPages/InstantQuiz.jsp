  ...
  $(document).ready(function(){
	...
	function countdown(course) {
		...
		cmin2=document.getElementById("Minutes").value;
	 	csec2=document.getElementById("Seconds").value;
	 	var time = cmin2 * 60  + csec2;
	 	if(cmin2==0 && csec2==0){
		 	alert("Kindly give quiz time");
		 	return false;
		 }
	 	//updateQuizTime();
	 	setSSE(time, resp, nunoptions);			 	
	}
	...
	function updateQuizTime(){		
		var noofoptions =noof;
		xmlhttp.onreadystatechange = function() {
			if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
			}		
		};	
		xmlhttp.open("GET", "../../InstantQuizTimeUpdate?optiontime="+ noofoptions + ":" + cmin2 + ":" + csec2, true);
		xmlhttp.send();
	}
	var noof =0;
	function setSSE(time, resp, nunoptions){
		getXMLhttp();
		xmlhttp.onreadystatechange = function() {
			if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
		...
		xmlhttp.open("POST","../../jsp/instructorJspPages/InstructorQuizSSE.jsp",true);
	    xmlhttp.setRequestHeader("Content-type","application/x-www-form-urlencoded");
	    xmlhttp.send("Status=startInstant&cid="+cid + "&time="+time + "&cor_resp="+resp + "&nunoptions="+nunoptions);		
	}
	...
	function stopSSE(){
		...
		xmlhttp.onreadystatechange = function() {
			if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
				isRightTimeToCollect();
			}		
		};	
		xmlhttp.open("GET", "../../jsp/instructorJspPages/InstructorQuizSSE.jsp?Status=stopInstant&cid="+cid, true);
		xmlhttp.send();
	}
	
	var checkidle;
	function isRightTimeToCollect(){
	...
	function checkIdle(){	
		...
		xmlhttp.onreadystatechange=function()
		{
			if (xmlhttp.readyState==4 && xmlhttp.status==200)
			{
				if(idleseconds>=3){
	            	...
	            	//collectResponse();
	            	window.location.href="../../jsp/instructorJspPages/instantresponseschart.jsp";
	            	//storeResponse();
	            }
				var participantCount = xmlhttp.responseText;
		...
		xmlhttp.open("GET","../../jsp/instructorJspPages/InstantQuizCheckIdle.jsp",true);
		xmlhttp.send();    
	}
	...
	function collectResponse(){
		...
		xmlhttp.open("GET", "../../jsp/instructorJspPages/InstantSSE.jsp?Status=resp", true);
		xmlhttp.send();
	}
...
<body>
<%@ include file="../../jsp/newMenu/newMenuwithCSS.jsp"%>
