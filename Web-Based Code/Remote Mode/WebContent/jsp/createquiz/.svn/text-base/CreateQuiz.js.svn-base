...
function deletequizenable(input, CourseID){
	...
	xmlhttp.onreadystatechange = function() {
		if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {			
			document.getElementById("QuizNameSelect").innerHTML = xmlhttp.responseText;
		}
	};
	if(input != "2"){
		var quiztype = 1;
		xmlhttp.open("GET", "../../jsp/createquiz/QuizNameSelect.jsp?CourseID=" + CourseID + "&quiztype=" + quiztype, true);
		xmlhttp.send();
	}else{
		var quiztype = 2;
		xmlhttp.open("GET", "../../jsp/createquiz/QuizNameSelect.jsp?CourseID=" + CourseID + "&quiztype=" + quiztype, true);
