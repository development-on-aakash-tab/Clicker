...
public class GenerateChartRemote extends HttpServlet {
	...
	protected void doProcess(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            if (chartType.equals("RemoteQuizResponse")){            	
            	...
            	for(int j=0;j<questionIDs.length;j++){
            		...
            		if(!questionIDs[j].equals("")){
	            		String Responses_Answer = reportHelper.getQuestionRemoteResponse(centerid,questionIDs[j],qid,qts);
            ...
            else if (chartType.equals("AllRemoteCenterResponse")){            	
            	ReportHelper reportHelper = new ReportHelper();
            	String qid = request.getParameter("qid");
            	int maxval=0;
            	String questionIDpatten = reportHelper.getQuestionIDs(qid);
            	...
            	for(int j=0;j<questionIDs.length;j++){
            		System.out.println(questionIDs[j]);
            		if(!questionIDs[j].equals("")){
	            		String Responses_Answer = reportHelper.getAllRCQuestionResponse(questionIDs[j],qid,quizRecordID);
	            		String allResponses = Responses_Answer.split("@")[0];
	            		System.out.println(allResponses);	            		
	            		if(!allResponses.equals("")){
	                	String responses[] = allResponses.split(";");	    
	                    ...
	                    String chartInfo[] = {"Question - " + (j+1), "Responses", "No of Responses"};
	                    responseChart(path, username, chartInfo, maxval, barDataset1,"Question"+(j+1), Responses_Answer.split("@")[1]);
            		}
            		}
            	}
            }
            	...
            	for(int j=0;j<questionIDs.length;j++){
            		System.out.println(questionIDs[j]);
            		if(!questionIDs[j].equals("")){
	            		String Responses_Answer = reportHelper.getRCQuestionResponse(centerid,questionIDs[j],qid,quizRecordID);
	            		String allResponses = Responses_Answer.split("@")[0];
	            		System.out.println(allResponses);	            		
	            		if(!allResponses.equals("")){
	                	String responses[] = allResponses.split(";");	    
