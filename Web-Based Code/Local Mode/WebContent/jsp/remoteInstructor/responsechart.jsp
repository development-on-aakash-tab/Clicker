...
String QuestionsIDs = reportHelper.getQuestionIDs(quizID);
int noofQuestions = QuestionsIDs.split("@").length;
System.out.println(centerid + quizID + QuestionsIDs);
%>
...
<body onload="showChart('<%=responseType%>','<%=centerid%>','<%=quizID%>', <%=noofQuestions%>, '<%=QuestionsIDs%>', '<%=InstructorID%>')">
	<center>
